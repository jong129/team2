...
public class ChatRagBlockedChunkCont {
    ...
    // (관리자) 차단 목록 조회
    @GetMapping
    public ResponseEntity<List<ChatRagBlockedChunkDto>> list(@RequestParam(name = "active", defaultValue = "true") boolean active, HttpSession session) {
    ...
    // (관리자) 단건 차단
    @PostMapping
    public ResponseEntity<ChatRagBlockedChunkDto> block(@RequestBody ChatRagBlockedChunkDto req, HttpSession session) {
        Long adminId = AuthSessionUtil.requireMemberId(session);
        // TODO AuthSessionUtil.requireAdmin(session);
    ...
    public ResponseEntity<ChatRagBlockedChunkDto> unblock(@PathVariable("chunkId") Long chunkId,HttpSession session) {
        Long adminId = AuthSessionUtil.requireMemberId(session);
        // TODO AuthSessionUtil.requireAdmin(session);
    ...
    public ResponseEntity<AutoBlockRes> autoBlockAll(
        ...
        // 싫어요 기준(badN) 이상인 답변들에서 자주 등장한 chunk를 통계로 뽑음
        ChatMessageRefDto dto = refService.badChunksAll(days, top, badN);
        
        // 그 결과에서 후보 chunkId 리스트 추출
        ...
        // 응답 AutoBlockRes
        return ResponseEntity.ok(new AutoBlockRes(blockedCount, chunkIds.size(), finalReason)); // 실제 반영된 수, 후보 수, 최종 사유
    }
