...
public class AdminPostDecisionService {
  ...
  private static final int POST_B_THRESHOLD = 80; // 위험도 80점 항목
  ...
  public AdminPostDecisionDebugResponse debugPostDecision(Long preSessionId) {
    ...
    if (preSession.getPhase() != ChecklistPhase.PRE || preSession.getStatus() != SessionStatus.COMPLETED) {
    ...
    // 2️⃣ NOT_DONE 항목 조회
    // =========================================================
    List<ChecklistItem> notDoneItems = itemRepository.findNotDoneItemsByPreSessionId(preSessionId);

    if (notDoneItems.isEmpty()) {
      // 미이행 항목이 없으면 무조건 POST_A
      return new AdminPostDecisionDebugResponse("POST_A", 0.0, List.of());
    }

    // =========================================================
    // 3️⃣ itemId → ChecklistItem 매핑
    // =========================================================
    Map<Long, ChecklistItem> itemMap = notDoneItems.stream()
        .collect(Collectors.toMap(ChecklistItem::getItemId, it -> it));

    // =========================================================
    // 4️⃣ AI 스코어 요청 DTO 생성
    // =========================================================
    ChecklistScoreRequest scoreRequest = new ChecklistScoreRequest();
    scoreRequest.setItems(notDoneItems.stream().map(it -> {
    ...
    try {
      ...
      return new AdminPostDecisionDebugResponse("POST_B", 0.0, List.of());
    }
    ...
    // =========================================================
    double riskScore = scoreResponse.getRiskScore() != null ? scoreResponse.getRiskScore() : 0.0;

    // =========================================================
    // 7️⃣ 관리자 UI 표시용 Row 생성
    // =========================================================
    List<AdminPostDecisionScoreRowDto> rows = scores.stream().map(s -> {

      ChecklistItem item = itemMap.get(s.getItemId());

      return new AdminPostDecisionScoreRowDto(s.getItemId(), item != null ? item.getTitle() : "(알 수 없음)",
          s.getImportanceScore() // ← 이미 0~100
      );
    }).toList();

    // =========================================================
    // 8️⃣ POST 분기 판단 (⭐ 기준 변경)
    // =========================================================
    int maxScore = scores.stream().map(ChecklistScoreResult::getImportanceScore).filter(Objects::nonNull)
        .max(Integer::compareTo).orElse(0);

    
    String postGroupCode = maxScore >= POST_B_THRESHOLD ? "POST_B" : "POST_A";

    // =========================================================
    // 9️⃣ 결과 반환
    // =========================================================
    return new AdminPostDecisionDebugResponse(postGroupCode, riskScore, rows);
  }
}
