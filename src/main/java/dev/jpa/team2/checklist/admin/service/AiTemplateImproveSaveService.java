...
public class AiTemplateImproveSaveService {
    ...
    public void saveDraft(
            Long baseTemplateId,
            AdminAiImproveSaveRequestDto request
    ) {

        /* ==========================
         * 1️⃣ 기준 템플릿 조회
         * ========================== */
        ChecklistTemplate baseTemplate =
                templateRepository.findById(baseTemplateId)
                        .orElseThrow(() ->
                                new IllegalArgumentException("기준 템플릿이 존재하지 않습니다.")
                        );
        ...
        newTemplate.setTemplateName(
                baseTemplate.getTemplateName() + " (AI 개선 초안)"
        );
        ...
        for (PreviewItemDto item : items) {
             ...
             * 3-1. ItemMaster 생성
             * ========================== */
            ChecklistItemMaster itemMaster = new ChecklistItemMaster();
            itemMaster.setPhase(newTemplate.getPhase().name());
            itemMaster.setPostGroupCode(newTemplate.getPostGroupCode());
            itemMaster.setTitle(item.getTitle());
            itemMaster.setDescription(item.getDescription());
            itemMaster.setActiveYn("Y");
