...
public class PreRiskExplanationAiService {
  ...
  public PreRiskExplanationDto generateExplanation(Double riskScoreSum, List<String> aiReasons) {
    ...
    dto.setActions(List.of("AI 분석 결과를 참고하여 계약 전 주요 위험 요소를 우선적으로 확인하시기를 권장드립니다."));

    return dto;
  }

  /**
   * 여러 AI reason을 종합해 최대 3줄 이내 요약 생성 (mock)
   */
  private String buildThreeLineSummary(Double riskScoreSum, List<String> aiReasons) {
    ...
    String riskLevel = riskScoreSum >= 70 ? "보증금 회수에 영향을 줄 수 있는" : "계약 전 확인이 필요한";

    return String.format("AI 분석 결과, %s 위험 요소가 반복적으로 발견되었습니다.\n" + "주요 원인은 권리관계 및 계약 주체 확인과 관련된 사항으로 나타났습니다.\n"
        + "계약 체결 전 해당 부분을 중심으로 재확인을 권장드립니다.", riskLevel);
  }
}
