...
public class PostChecklistService {
  ...
  private final ChecklistSatisfactionRepository satisfactionRepo;
  ...
  public PostChecklistDTO startPostSession(Long memberId, Long preSessionId) {
    ...
    Optional<ChecklistSession> existing = sessionRepo
        .findTopByMemberIdAndPhaseAndStatusAndDeletedYnOrderByStartedAtDesc(memberId, Phase.POST, "IN_PROGRESS", "N");
    ...
    if (existing.isPresent()) {
      ...
      return PostChecklistDTO.builder().sessionId(s.getSessionId()).templateId(templateId)
          .postGroupCode(full.getPostGroupCode()).profileKeyId(s.getProfileKeyId()).build();
    }
    ...
    if (preOpt.isEmpty()) {
      ...
      UserPreProfileKey pk = profileKeyRepo.findByKeyHash(keyHash).orElseGet(() -> profileKeyRepo
          .save(UserPreProfileKey.builder().keyHash(keyHash).keyJson(keyJson).createdAt(LocalDateTime.now()).build()));

      String postGroupCode = route(signals); // => POST_A

      Long postTemplateId = postTemplateRepo
          .findTopByPhaseAndPostGroupCodeAndStatusOrderByVersionNoDesc(Phase.POST, postGroupCode, TemplateStatus.ACTIVE)
          .orElseThrow(() -> new IllegalArgumentException("ACTIVE POST 템플릿 없음: " + postGroupCode)).getTemplateId();

      ChecklistSession postSession = ChecklistSession.builder().memberId(memberId).phase(Phase.POST)
          .template(ChecklistTemplate.builder().templateId(postTemplateId).build()).profileKeyId(pk.getProfileKeyId()) // ✅
                                                                                                                       // null
                                                                                                                       // 금지
                                                                                                                       // ->
                                                                                                                       // 제약조건
                                                                                                                       // 통과
          .status("IN_PROGRESS").startedAt(LocalDateTime.now()).deletedYn("N").build();

      ChecklistSession saved = sessionRepo.save(postSession);

      return PostChecklistDTO.builder().sessionId(saved.getSessionId()).templateId(postTemplateId)
          .postGroupCode(postGroupCode).profileKeyId(pk.getProfileKeyId()).build();
    }
    ...
    /* 3) PRE 핵심 문항 조회 */
    List<ChecklistItem> signalItems = itemRepo.findByTemplate_TemplateIdAndItemOrderIn(preTemplateId, SIGNAL_ORDERS);
    ...
    /* 4) PRE 응답 조회 */
    List<ChecklistResponse> preResponses = responseRepo.findBySession_SessionId(preSession.getSessionId());
    ...
    Long postTemplateId = assignmentRepo
        ...
        .orElseGet(() -> postTemplateRepo
            .findTopByPhaseAndPostGroupCodeAndStatusOrderByVersionNoDesc(Phase.POST, postGroupCode,
  ...
  private String getStatus(Map<Integer, Long> orderToItemId, Map<Long, String> itemIdToStatus, int order) {
    ...
    if (itemId == null)
      return CheckStatus.NOT_DONE.name();
    return itemIdToStatus.getOrDefault(itemId, CheckStatus.NOT_DONE.name());
  }

  private String route(Map<String, String> s) {
    if (isNotDone(s.get("GAPGU_RISK")) || isNotDone(s.get("TRUST")))
      return POST_D;
    if (isNotDone(s.get("EULGU_MORTGAGE")) || isNotDone(s.get("PRIOR_SAFE")))
      return POST_C;
    if (isNotDone(s.get("BUILDING_ILLEGAL")) || isNotDone(s.get("MOVEIN_OK")))
      return POST_B;
    return POST_A;
  }
  ...
  private String toStableJson(Map<String, String> signals) {
    String body = signals.entrySet().stream().map(e -> "\"" + e.getKey() + "\":\"" + e.getValue() + "\"")
  ...
  public PostChecklistResponseDTO getPostChecklist(Long sessionId) {
    ...
    List<PostChecklistItemDTO> itemDTOs = items.stream()
        .map(it -> PostChecklistItemDTO.builder().itemId(it.getItemId()).itemOrder(it.getItemOrder())
            .checkArea(it.getCheckArea()).title(it.getTitle()).description(it.getDescription())
            .requiredYn(it.getRequiredYn()).build())
        .toList();

    return PostChecklistResponseDTO.builder().sessionId(sessionId).templateId(template.getTemplateId())
        .templateName(template.getTemplateName()).items(itemDTOs).build();
  }
  ...
  public List<PostChecklistStatusDTO> getPostStatuses(Long sessionId) {
    ...
    return responses.stream().map(
        r -> PostChecklistStatusDTO.builder().itemId(r.getItem().getItemId()).checkStatus(r.getCheckStatus()).build())
        .toList();
  }
  ...
  public void updateCheckStatus(Long sessionId, Long itemId, String checkStatus) {
    ...
    if (checkStatus == null)
      throw new IllegalArgumentException("checkStatus가 없습니다.");
    ...
    ChecklistResponse resp = responseRepo.findBySession_SessionIdAndItem_ItemId(sessionId, itemId)
        .orElseGet(() -> ChecklistResponse.builder().session(ChecklistSession.builder().sessionId(sessionId).build())
            .item(ChecklistItem.builder().itemId(itemId).build()).build());
  ...
  public Page<ChecklistHistoryRowDTO> getPostHistoryDto(Long memberId, int page, int size) {
    ...
    Page<ChecklistSession> p = sessionRepo.findByMemberIdAndPhaseAndDeletedYn(memberId, Phase.POST, "N", pageable);

    return p.map(s -> ChecklistHistoryRowDTO.builder().sessionId(s.getSessionId()).phase(s.getPhase().name())
        .status(s.getStatus()).startedAt(s.getStartedAt()).completedAt(s.getCompletedAt()).build());
  }
  ...
  public PostChecklistSummaryDTO getSummary(Long sessionId) {
    ...
    if (items.isEmpty()) {
      return PostChecklistSummaryDTO.builder().level("EMPTY").message("표시할 체크리스트 항목이 없습니다. (템플릿/아이템 확인 필요)").build();
    }
    ...
    long requiredNotDone = items.stream().filter(i -> "Y".equals(i.getRequiredYn()))
        .filter(i -> "NOT_DONE".equals(statusMap.getOrDefault(i.getItemId(), "NOT_DONE"))).count();
    ...
    return PostChecklistSummaryDTO.builder().level(level).message(message).build();
  }
  ...
  public void saveSatisfaction(Long sessionId, Integer rating, String commentText) {
    ...
    // ✅ 권장: COMPLETED 세션만 만족도 받기
    if (!"COMPLETED".equalsIgnoreCase(String.valueOf(session.getStatus()))) {
      throw new IllegalStateException("session is not completed");
    }

    satisfactionRepo.findBySession_SessionId(sessionId).ifPresentOrElse(existing -> {
      existing.setRating(rating);
      existing.setCommentText(commentText);
      // createdAt은 최초 작성일 유지하고 싶으면 그대로 두기
    }, () -> {
      ChecklistSatisfaction sat = ChecklistSatisfaction.builder().session(session).rating(rating)
          .commentText(commentText).createdAt(java.time.LocalDateTime.now()).build();
      satisfactionRepo.save(sat);
    });
  }

  @Transactional(readOnly = true)
  public PostChecklistSatisfactionDTO getSatisfaction(Long sessionId) {
    return satisfactionRepo.findBySession_SessionId(sessionId)
        .map(s -> PostChecklistSatisfactionDTO.builder().sessionId(sessionId).rating(s.getRating())
            .commentText(s.getCommentText()).createdAt(s.getCreatedAt() == null ? null : s.getCreatedAt().toString())
            .build())
        .orElse(null);
  }
