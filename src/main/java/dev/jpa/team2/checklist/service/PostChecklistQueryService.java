...
public class PostChecklistQueryService {
  ...
  private final ChecklistSatisfactionRepository satisfactionRepository;
     ...
     * ✅ POST 체크리스트 기록 조회
     */
  public Page<PostChecklistHistoryRowDto> getPostHistory(
      Long memberId,
      ...
      return sessionRepository
          ...
          .map(s -> new PostChecklistHistoryRowDto(
              s.getSessionId(),
              s.getStatus(),
              s.getStartedAt(),
              s.getCompletedAt()
  ...
  public PostChecklistDto getPostChecklist(Long sessionId) {
      ...
      ChecklistSession session = sessionRepository.findById(sessionId)
          .orElseThrow(() ->
              new IllegalStateException("POST 세션이 존재하지 않습니다.")
          );

      // 2️⃣ 템플릿 조회 (이름용)
      ChecklistTemplate template = templateRepository.findById(session.getTemplateId())
          .orElseThrow(() ->
              new IllegalStateException("POST 템플릿이 존재하지 않습니다.")
          );

      // 3️⃣ 🔑 세션에 복제된 CHECKLIST_ITEM 조회 (중요)
      List<ChecklistItem> sessionItems =
          itemRepository.findBySessionIdOrderByItemOrderAsc(sessionId);

      // 4️⃣ 프론트용 DTO 변환
      List<PostChecklistItemDto> items = sessionItems.stream()
          .map(item -> new PostChecklistItemDto(
              item.getItemId(),      // ✅ CHECKLIST_ITEM.ITEM_ID
              item.getCheckArea(),
              item.getTitle(),
              item.getDescription(),
              item.getRequiredYn()
  ...
  public PostChecklistSatisfactionDto getSatisfaction(Long sessionId) {
      return satisfactionRepository.findBySessionId(sessionId)
          .map(entity -> {
